...
class CAP2BNetv2(TwoStageDetector):
    def __init__(self,
                 backbone,
                 roi_head,
                 train_cfg,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(CAP2BNetv2, self).__init__(
            backbone=backbone,
            neck=neck,
            roi_head=roi_head,
            train_cfg=train_cfg,
    ...
    def forward_train(self,
        ...
        for stage in range(self.num_stages):
            if stage == 0:
                ...
                img_dynamic_weight = torch.ones(len(img), device='cuda')      # changed by guo，先另两个阶段的dynamic weight都是1
                neg_proposal_list, neg_weight_list = None, None
                pseudo_boxes = generate_proposals
                pred_labels = gt_labels  # first stage
            else:
                ...
                neg_proposal_list, neg_weight_list = gen_negative_proposals(gt_points, fine_proposal_cfg,
                                                                            generate_proposals,
                                                                            img_meta=img_metas)
                # dynamic_weight_ = torch.ones(len(img), device='cuda')      # changed by guo，先另两个阶段的dynamic weight都是1
            roi_losses, pseudo_boxes, dynamic_weight, img_dynamic_weight, pred_labels  = self.roi_head.forward_train(stage, x, img_metas,
                                                                                   pseudo_boxes,
                                                                                   generate_proposals,
                                                                                   proposals_valid_list,
                                                                                   neg_proposal_list, neg_weight_list,
                                                                                   gt_true_bboxes, gt_labels, pred_labels,
                                                                                   dynamic_weight,
                                                                                   img_dynamic_weight,
