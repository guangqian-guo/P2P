...
class SAM_PRNetv7(TwoStageDetector):
    def __init__(self,
                 backbone,
                 roi_head,
                 train_cfg,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(SAM_PRNetv7, self).__init__(
            backbone=backbone,
            neck=neck,
            roi_head=roi_head,
            train_cfg=train_cfg,
    ...
    def forward_train(self,
        ...
        idx = []
        _ = 0
        for sam_id in range(len(gt_labels)):
            for label in range(len(gt_labels[sam_id])):
                bc_id.append(sam_id)
                idx.append(_)
                _ += 1

        bc_id = torch.tensor(bc_id, device=gt_labels[0].device)
        idx  = torch.tensor(idx, device=gt_labels[0].device)

        gt_labels_ = torch.cat(gt_labels)
        gt_labels_with_bc = torch.hstack((idx.reshape(-1,1), gt_labels_.reshape(-1,1), bc_id.reshape(-1,1)))
        # print(gt_labels_with_bc)
        label_unique = torch.unique(gt_labels_with_bc[:,1])
        unique_id = []
        for label_uni in label_unique:
            label_points = gt_labels_with_bc[gt_labels_with_bc[:, 1] == label_uni]
            # print(label_points)
            uni_ele, uni_count = torch.unique(label_points[:, 2], return_counts=True)
            # print(uni_ele, uni_count)
            unique_single_indices = torch.where(uni_count == 1)[0]
            # print(unique_single_indices)
            if len(unique_single_indices) > 0:
            # 找到在原始张量中对应的索引
                for unique_single_indice in unique_single_indices:
                    single_indices = label_points[torch.nonzero(label_points[:, 2] == uni_ele[unique_single_indice], as_tuple=False)][:,:, 0].reshape(-1)
                    # print(single_indices)
                unique_id.append(single_indices[0].item())
        unique_labels = gt_labels_[unique_id]
        num_pps_per_point_0st = len(base_proposal_cfg['base_scales'] * len(base_proposal_cfg['base_ratios']))
        num_pps_per_point_1st = len(fine_proposal_cfg['base_ratios']) * len(fine_proposal_cfg['base_ratios']) *5 if fine_proposal_cfg['shake_ratio'] else len(fine_proposal_cfg['base_ratios']) * len(fine_proposal_cfg['base_ratios'])
        proposal_id_1st = torch.tensor([i * num_pps_per_point_1st for i in unique_id], device=unique_labels.device)
        proposal_id_0st = torch.tensor([i * num_pps_per_point_0st for i in unique_id], device=unique_labels.device)
        unique_id = torch.hstack((proposal_id_1st.reshape(-1,1), unique_labels.reshape(-1,1), proposal_id_0st.reshape(-1,1)))
        # print(unique_id)
        # ---------------------------------------------------------------------------
        ...
        for stage in range(self.num_stages):
            ...
            roi_losses, pseudo_boxes, filtered_bboxes, dynamic_weight = self.roi_head.forward_train(stage, x, img_metas,
                                                                                   ...
                                                                                   gt_true_bboxes, gt_labels, unique_id,
                                                                                   dynamic_weight,
