...
# Data: 2023/7/10
# HMIL   base proposal add differnent length width ratio
...
def gen_proposals_from_seed(seed, proposal_cfg, img_meta):
    ...
    # aspect_ratios = proposal_cfg['aspect_ratio']
    ...
    for i in range(len(img_meta)):
        ...
        base_boxes = seed[i]
        # show_proposals(base_boxes.unsqueeze(0), img_meta[i])
        for scale_w in base_scale:
            for scale_h in base_scale:
                coarse_cluster = []
                # add base proposal
                for ratio in base_ratios:
                    base_boxes_ = bbox_xyxy_to_cxcywh(base_boxes)
                    base_boxes_[:, 2] *= (ratio * scale_w )
                    base_boxes_[:, 3] *= (ratio * scale_h)
        ...
        # show_proposals(pps[0], img_meta[i])
        
        pps = torch.cat(pps, dim=1)
        
        proposals_valid = pps.new_full(
            (pps.shape[0], len(base_scale)*len(base_scale), 1), 1, dtype=torch.long).reshape(-1, 1)  # TODO !!! 3 is num_cluster!!!!
...
def show_proposals(proposals, img_meta):
    ...
    for i in range(len(boxes)):
        ...
        for j in range(len(boxes[i])):
            ...
            # igs1 = cv2.addWeighted(igs1, 1.0, blk, 0.1, 1, dst=None, dtype=None)
            igs1 = cv2.rectangle(igs1, (boxes[i, j, 0], boxes[i, j, 1]), (boxes[i, j, 2], boxes[i, j, 3]),
                                    color=color, thickness=2)

    save_dir = '/home/ps/Guo/debug/'
...
class SAM_PRNetv6(TwoStageDetector):
    def __init__(self,
                 backbone,
                 roi_head,
                 train_cfg,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(SAM_PRNetv6, self).__init__(
            backbone=backbone,
            neck=neck,
            roi_head=roi_head,
            train_cfg=train_cfg,
