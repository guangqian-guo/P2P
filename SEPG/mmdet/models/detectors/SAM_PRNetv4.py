...
class SAM_PRNetv4(TwoStageDetector):
    def __init__(self,
                 backbone,
                 roi_head,
                 train_cfg,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(SAM_PRNetv4, self).__init__(
            backbone=backbone,
            neck=neck,
            roi_head=roi_head,
            train_cfg=train_cfg,
    ...
    def forward_train(self,
        ...
        for stage in range(self.num_stages):
            ...
            roi_losses, pseudo_boxes, dynamic_weight = self.roi_head.forward_train(stage, x, img_metas,
                                                                                   ...
                                                                                   **kwargs)
            # for bs in range(len(pseudo_boxes)):
            #     bs_pse_bbox = pseudo_boxes[bs]
            #     bs_gt_bbox = gt_bboxes[bs]
            #     iou = bbox_overlaps(bs_pse_bbox, bs_gt_bbox)
            #     for i in range(bs_pse_bbox.shape[0]):
            #         pse_bbox = bs_pse_bbox[i].unsqueeze(0)
            #         gt_bbox = bs_gt_bbox[i].unsqueeze(0)
            #         iou = bbox_overlaps(pse_bbox, gt_bbox)
            #         print(iou)
            # exit()
            if stage == 0:
    ...
    def simple_test(self, img, img_metas, gt_bboxes, gt_anns_id, gt_true_bboxes, gt_labels,
        ...
        for stage in range(self.num_stages):
            ...
            # a = [[] for _ in range(len(pseudo_boxes))]
            # for bs in range(len(pseudo_boxes)):
            #     bs_pse_bbox = pseudo_boxes[bs]
            #     bs_gt_bbox = gt_bboxes[bs]
            #     iou = bbox_overlaps(bs_pse_bbox, bs_gt_bbox)
