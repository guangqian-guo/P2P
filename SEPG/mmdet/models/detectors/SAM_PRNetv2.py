    ...
    for i in range(len(img_meta)):
        ...
        for scale in base_scale:
            for ratio in base_ratios:
                base_boxes_ = bbox_xywh_to_xyxy(base_boxes)
                base_boxes_ = bbox_xyxy_to_cxcywh(base_boxes_)
        ...
        if shake_ratio is not None:
            pps_new = []
            pps_new.append(pps_old.reshape(*pps_old.shape[0:2], -1, 4))  # 先把pps_old append
            for ratio in shake_ratio:
                pps = bbox_xyxy_to_cxcywh(pps_old)
                pps_center = pps[:, :, :2]  # 把中心点和wh分开，抖动的时候只改变中心点
                pps_wh = pps[:, :, 2:4]
                ...
                pps = torch.cat([pps_center, pps_wh], dim=-1)    # 抖动完成后和wh再进行cat
...
class SAM_PRNetv2(TwoStageDetector):
    def __init__(self,
                 backbone,
                 roi_head,
                 train_cfg,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(SAM_PRNetv2, self).__init__(
            backbone=backbone,
            neck=neck,
            roi_head=roi_head,
            train_cfg=train_cfg,
    ...
    def forward_train(self,
        ...
        for stage in range(self.num_stages):
            if stage == 0:
                generate_proposals, proposals_valid_list = fine_proposals_from_cfg(gt_bboxes, base_proposal_cfg,
