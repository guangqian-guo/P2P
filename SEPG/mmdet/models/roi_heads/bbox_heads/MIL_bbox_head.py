...
# added by guo -------------------------------------------------------------------------------------------------- #
@HEADS.register_module()
class Shared2FCInstanceMILHead_refine(BBoxHead):

    def __init__(self,
                 num_stages=1,
                 num_shared_fcs=2,
                 num_cls_refine_fcs=2,
                 num_cls_fcs=0,
                 num_reg_fcs=0,
                 num_ref_fcs=0,
                 conv_out_channels=256,
                 ...
                 loss_type=None,
                 with_loss_pseudo=True,
                 with_cls_refine=True,
                 loss_mil1=dict(
                     type='MILLoss',
                     binary_ins=False,
                     loss_weight=0.25),  # weight
                 loss_mil2=None,
                 loss_ins=None,  # weight, need to adjust
                 conv_cfg=None,
                 norm_cfg=None,
                 init_cfg=dict(
                     type='Normal',
                 ...
                 *args,
                 **kwargs):
        super(Shared2FCInstanceMILHead_refine, self).__init__(
            *args, init_cfg=init_cfg, **kwargs)
        assert (num_shared_fcs + num_cls_fcs + num_reg_fcs > 0)

        self.with_reg = False ##################################
        self.with_cls_refine = with_cls_refine
        ...
        self.num_reg_fcs = num_reg_fcs
        self.num_ref_fcs = num_ref_fcs
        self.num_cls_refine_fcs = num_cls_refine_fcs
        self.conv_out_channels = conv_out_channels
        self.fc_out_channels = fc_out_channels
        self.conv_cfg = conv_cfg
        self.norm_cfg = norm_cfg
        ...
        # add cls refine branch
        _, self.cls_refine_fcs, self.cls_refine_last_dim = \
            self._add_conv_fc_branch(0, self.num_cls_refine_fcs, self.shared_out_channels)
        ...
        self.fc_ins = nn.ModuleList()
        self.fc_reg = nn.ModuleList()
        self.fc_cls_refine = nn.ModuleList()
        for i in range(self.num_stages):
            if i < 1:
                num_cls = self.num_classes + 1
                # added by guo for cls branch refine
                if self.with_cls_refine:
                    self.fc_cls_refine.append(build_linear_layer(
                        self.cls_predictor_cfg,
                        in_features=self.cls_refine_last_dim,
                        out_features=num_cls))
            else:
                num_cls = self.num_classes
            if self.with_cls:
    ...
    def forward(self, x, stage):
        ...
        if stage == 0:
            ...
            # added by guo
            for fc in self.cls_refine_fcs:
                x_cls_refine = self.relu(fc(x_cls_refine))
            cls_refine_score = self.fc_cls_refine[stage](x_cls_refine) if self.with_cls_refine else None
        ...
        return cls_score, ins_score, reg_box, cls_refine_score


    @force_fp32(apply_to=('cls_refine_score'))
    def loss_instance(self,
        ...
        if cls_refine_score is not None:
            
            avg_factor = max(torch.sum(label_weights > 0).float().item(), 1.)
            label_weights = torch.repeat_interleave(label_weights, cls_refine_score.shape[1])
            if cls_refine_score.numel() > 0:
                label_valid = proposals_valid_list
                cls_refine_score = cls_refine_score.softmax(dim=-1)
                num_sample = cls_refine_score.shape[0]
                pos_loss, bag_acc, num_pos = self.loss_ins(
                    cls_refine_score,
                    labels,
                    pse_labels,
                    label_valid,
                ...
                if isinstance(pos_loss, dict):
                    ...
                    losses['loss_instance'] = pos_loss
                losses['instance_acc'] = bag_acc
                ...
                if cls_score is not None:
                    if cls_score.numel() > 0:
                        ...
                        if isinstance(pos_loss, dict):
                            ...
                            losses['loss_img_mil'] = pos_loss
                        losses['img_bag_acc'] = bag_acc
                if self.with_loss_pseudo and stage == self.num_stages - 1:
                    cls_score_pseudo = cls_score[:, 0]
                    label_valid = cls_score.new_full(
                        (cls_score.shape[0], 1), 1, dtype=torch.long)
