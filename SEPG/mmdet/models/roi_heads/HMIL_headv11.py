...
class HMILHeadv11(StandardRoIHead):
    """Simplest base roi head including one bbox head and one mask head."""

    def __init__(self, bbox_roi_extractor, num_stages, bbox_head, top_k1=1, top_k2=7, with_atten=None, **kwargs):
        super(HMILHeadv11, self).__init__(bbox_roi_extractor=bbox_roi_extractor, bbox_head=bbox_head, **kwargs)
        self.threshold = 0.3
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            bbox_results = self._bbox_forward_train(x, proposal_list_base, proposals_list, proposals_valid_list,
                                                    neg_proposal_list,
                                                    neg_weight_list,
                                                    gt_points, gt_labels, unique_id, dynamic_weight,
                                                    img_metas, stage)

            losses.update(bbox_results['loss_instance_mil'])
        return losses, bbox_results['pseudo_boxes'], None,  bbox_results['dynamic_weight']
    ...
    def _bbox_forward_train(self, x, proposal_list_base, proposals_list, proposals_valid_list, neg_proposal_list,
        ...
        # proposal_list: 是一个列表，里面num_batch个tensor[N, 4]，分别对应每张图片提取的proposal
        # proposals_valid_list: 也是列表，可能是表示proposals_list中各个proposal是否有效的标志。
        # print(proposals_list)
        # print(proposals_valid_list)
        
        rois = bbox2roi(proposals_list)   # roi: N*5 的一个tensor，第一列是batch_id，N是整个batch所有图片中的目标总和
        # if stage == 1:
        #     print(len(proposals_list))
        #     print([proposals_list[i].shape for i in range(len(proposals_list))])
        #     print(rois.shape)
        gt_labels = torch.cat(gt_labels)
        
        
        bbox_results = self._bbox_forward(x, rois, gt_points, gt_labels, unique_id, stage) # 得到box results
        ...
        if neg_proposal_list is not None:
            ...
            neg_bbox_results = self._bbox_forward(x, neg_rois, None, None,  None, stage)  ######stage
        ...
        loss_instance_mil = self.bbox_head.loss_mil(stage, bbox_results['cls_score'], bbox_results['similar'],
                                                    proposals_valid_list,
    ...
    def _bbox_forward(self, x, rois, gt_points, gt_labels, unique_id, stage):
        """Box head forward function used in both training and testing."""
        # TODO: a more flexible way to decide which feature maps to use
        # 通过roialign 提取roi features，因为输入的是多尺度的特征，所以里面有个roi_map函数，把roi分配到不同的金字塔level。
        # 所以，roi的数量是42*(n1+n2+...nn)，box_features的数量也是相同。
        
        bbox_feats = self.bbox_roi_extractor(                 
            x[:self.bbox_roi_extractor.num_inputs], rois)   
        # if stage == 1:
        #     print(rois.shape)
        #     print(bbox_feats.shape)   # [42xnum_gts, 256, 7, 7]
        
        if self.with_shared_head:  # self.with_share_head==False
            bbox_feats = self.shared_head(bbox_feats)

        
        cls_score, similar, reg_box = self.bbox_head(bbox_feats, stage, gt_points, gt_labels, unique_id)
        ...
        if gt_points is not None:
            ...
            cls_score = cls_score.view(num_gt, -1, cls_score.shape[-1])   # 原本是两个batch所有point都stack在一起，现在把各个point对应的proposal分开。
            # if ins_score is not None:
            #     ins_score = ins_score.view(num_gt, -1, ins_score.shape[-1])
            
            if stage == 0:
                cls_scores_split = torch.chunk(cls_score, chunks=num_clusters, dim=1)
                # ins_scores_split = torch.chunk(ins_score, chunks=num_clusters, dim=1)
                avg_cls_scores = [torch.mean(cluster, dim=1, keepdim=True) for cluster in cls_scores_split]
                # avg_ins_scores = [torch.mean(cluster, dim=1, keepdim=True) for cluster in ins_scores_split]
                cls_score = torch.cat(avg_cls_scores, dim=1)
                # ins_score = torch.cat(avg_ins_scores, dim=1)

            if reg_box is not None:
                reg_box = reg_box.view(num_gt, -1, reg_box.shape[-1])
            
            bbox_results = dict(
                cls_score=cls_score, similar=similar, bbox_pred=reg_box, bbox_feats=bbox_feats, num_instance=num_gt)
            return bbox_results
        # negative sample
        else:
            bbox_results = dict(
                cls_score=cls_score, similar=similar,  bbox_pred=reg_box, bbox_feats=bbox_feats, num_instance=None)
            return bbox_results

    # **
    def merge_box_single(self, cls_score, dynamic_weight, gt_point, gt_label, proposals, img_metas, stage, ins_score=None):
        num_pros = 7
        if stage < self.num_stages - 1:
        ...
        if stage == 0:
            proposals = proposals.reshape(cls_score.shape[0], cls_score.shape[1]*num_pros, 4)  # TODO !!
        else:
            proposals = proposals.reshape(cls_score.shape[0], cls_score.shape[1], 4)  # TODO !!
        ...
        if merge_mode == 'weighted_clsins_topk':
            if stage == 0:
                ...
                k = self.topk2
            dynamic_weight_, idx = dynamic_weight.topk(k=k, dim=1)  # synamic weight is the score of ins*cls
            ...
            if stage == 0: 
                ...
                indices = torch.arange(num_pros, device='cuda') + start
                filtered_boxes = proposals[torch.arange(proposals.shape[0]).unsqueeze(1), indices, :]
                ...
                boxes = (filtered_boxes).mean(dim=1)
                # print(boxes)
            else:
                ...
                filtered_scores = dict(cls_score=cls_score[torch.arange(proposals.shape[0]).unsqueeze(1), idx],
                                   ins_score=None,
                                   dynamic_weight=dynamic_weight_)
            # print(filtered_boxes.shape)  # [N, topk, 4]
    ...
    def merge_box(self, bbox_results, proposals_list, proposals_valid_list, gt_labels, gt_bboxes, img_metas, stage):
        ...
        similar = bbox_results['similar']
        num_instances = bbox_results['num_instance']  # num_gt * num_box * num_class
        
        if stage == 0:
            _, __, num_classes = cls_scores.shape
            cls_scores = cls_scores.view(cls_scores.shape[0], -1).softmax(dim=-1).view(cls_scores.shape[0], -1, num_classes)
            
            ins_scores = similar.softmax(dim=-2)
            
            # ins_scores = F.normalize(ins_scores, dim=1, p=1)
            dynamic_weight = (cls_scores)
            dynamic_weight = dynamic_weight[torch.arange(len(cls_scores)), :, gt_labels]
            
            cls_scores = cls_scores[torch.arange(len(cls_scores)), :, gt_labels]   # gt_labels 是已知的，所以把对应的得分提出来。如果point是类不可知的，那么得选定一个proposal prototype,确定类别和得分高低。
            ins_scores = ins_scores[torch.arange(len(cls_scores)), :, gt_labels]
            # split batch
            batch_gt = [len(b) for b in gt_bboxes]
            cls_scores = torch.split(cls_scores, batch_gt)  # cls_score: 是一个tuple，应该是batch个score
            ...
            boxes, filtered_boxes, filtered_scores = multi_apply(self.merge_box_single, cls_scores, 
                                                                dynamic_weight_list,
                                                                gt_bboxes,
                                                                gt_labels,
                                                                proposals_list,
                                                                img_metas, stage_, ins_scores)
        else:
            _, __, num_classes = cls_scores.shape
            cls_scores = cls_scores.sigmoid()

            # ins_scores = similar.softmax(dim=-2) * proposals_valid_list
            # ins_scores = F.normalize(ins_scores, dim=1, p=1)
        ...
        # self.show_box(proposals_list, filtered_scores,filtered_boxes)
    ...
    def simple_test(self,
        ...
        return bbox_results, pseudo_bboxes, None
    ...
    def simple_test_bboxes(self,
        ...
        rois = bbox2roi(proposals)
        bbox_results = self._bbox_forward(x, rois, gt_bboxes, gt_labels, None, stage)
        proposals_valid_list = torch.cat(proposals_valid_list).reshape(
            *bbox_results['cls_score'].shape[:2], 1)
        ...
        pseudo_boxes, mean_ious, filtered_boxes, filtered_scores, dynamic_weight = self.merge_box(bbox_results,
                                                                                                  ...
                                                                                                  gt_bboxes,
                                                                                                  img_metas, stage)
        # print(len(pseudo_boxes), pseudo_boxes[0].shape)
        ...
        # print(len(det_bboxes), det_bboxes[0].shape)
        # exit()
