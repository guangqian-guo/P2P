...
pse_gt_path = 'data/COCO/point_ann/instances_train2017_10_percent_pse_ann.json'
pse_gt_path = '/home/ubuntu/Guo/GroupRCNN-main/instances_train2017_10_bottem_pse_ann.json'
gt_path = '/home/ubuntu/Guo/P2BNet-main/TOV_mmdetection/data/COCO/annotations/instances_train2017.json'
...
for x in pse_gt['images']:
    try:
        if 'with_bbox_ann' in x.keys() and x['with_bbox_ann']:
...
img2id = defaultdict()
for x in pse_gt['images']:
    if x['file_name'] in show_img[:1000]:
...
for img_name in show_img[:1000]:
    ...
    if gt_or_result == 1:
        plt.savefig(os.path.join('vis_bottom10_pse_bbox_' + name_out, img_name), bbox_inches='tight')
    else:
        plt.savefig(os.path.join('vis_result_bbox_' + name_out, img_name), bbox_inches='tight')
